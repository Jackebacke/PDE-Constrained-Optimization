...
############################################################################
##                                                                                  
##  SBP operators for "PDE-constrained optimization" at Uppsala University. 
##
##                                                                                  
##  Authors: Gustav Eriksson, Martin Almquist                                                        
##  Date:   2026-08-08                                                              
##                                                                                  
...
#   gamma - "borrowing" constant needed for SAT boundary conditions
# 
# Use as follows:
# 
# import sbp_operators as ops
# H,HI,D1,D2,e_l,e_r,d1_l,d1_r,gamma = ops.sbp_cent_2nd(m,h,order)
def sbp_cent_2nd(m,h):
    ...
    return H,HI,D1,D2,e_l,e_r,d1_l,d1_r,gamma
...
# 
# import sbp_operators as ops
# H,HI,D1,D2,e_l,e_r,d1_l,d1_r,gamma = ops.sbp_cent_4th(m,h,order)
...
# 
# import sbp_operators as ops
# H,HI,D1,D2,e_l,e_r,d1_l,d1_r,gamma = ops.sbp_cent_6th(m,h,order)
...
#   H - inner product matrix
#   HI - inverse of H
#   Dp - right-biased first derivative SBP operator
#   Dm - left-biased derivative SBP operator
#   e_l,e_r - vectors to extract the boundary grid points
#   gamma - "borrowing" constant needed for SAT boundary conditions for DpDm
# 
# Use as follows:
# 
# import sbp_operators as ops
# H,HI,Dp,Dm,e_l,e_r,gamma = ops.sbp_upwind_5th(m,h,order)
def sbp_upwind_5th(m, h):
    ...
    H[:4,:4] = np.array([[0.251e3 / 0.720e3, 0, 0, 0], 
    [0, 0.299e3 / 0.240e3, 0, 0], 
    [0, 0, 0.211e3 / 0.240e3, 0], 
    [0, 0, 0, 0.739e3 / 0.720e3]])

    H[-4:,-4:] = np.fliplr(np.flipud(H[:4,:4]))
    ...
    Qp=(1/20*np.diag(np.ones(m-2),-2)-1/2*np.diag(np.ones(m-1),-1)-1/3*np.diag(np.ones(m),0)+1*np.diag(np.ones(m-1),+1)-1/4*np.diag(np.ones(m-2),+2)+1/30*np.diag(np.ones(m-3),+3))

    Q_U = np.array([[-0.1e1 / 0.120e3, 0.941e3 / 0.1440e4, -0.47e2 / 0.360e3, -0.7e1 / 0.480e3],
    ...
    Qp[:4,:4]=Q_U
    Qp[-4:,-4:]=np.flipud( np.fliplr(Q_U[:4,:4] ) ).T # This is different from standard SBP
    ...
    H = spsp.csc_matrix(H)
    HI = spsp.csc_matrix(HI)
    Dp = spsp.csc_matrix(Dp)
    Dm = spsp.csc_matrix(Dm)
    e_l = spsp.csc_matrix(e_l).transpose()
    e_r = spsp.csc_matrix(e_r).transpose()
    return H,HI,Dp,Dm,e_l,e_r,gamma